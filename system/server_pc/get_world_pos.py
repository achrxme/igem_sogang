...
def get_world_pos():
	...
	top_cam = VideoStream(src=0).start()
	# gripper_cam = VideoStream(src=0).start()
	time.sleep(2.0)
	...
	while True:
		...
		if len(corners) > 0 :
			...
			if min(len(x[0]), len(x[1]), len(x[2]), len(x[3]), len(x[4]), len(x[5]), len(x[6])) > 10:
				break
...
def adjust_world_pos():
	...
	print("[INFO] starting video stream...")
	gripper_cam = VideoStream(src=1).start()
	time.sleep(2.0)
	...
	while True:
		...
		if len(corners) > 0 :
			...
			for (markerCorner, markerID) in zip(corners, ids):
				...
				if len(x_offset) <= 20:
					x_offset.append(topRight[0])
					y_offset.append(topRight[1])
	...
	mean_x_offset = np.mean(x_offset)
	mean_y_offset = np.mean(y_offset)
	...
	#scale adjust & offset to center
	x_result = -1*int(mean_x_offset*scale_coef + ADJUST_CENTER_X)
	y_result = int(mean_y_offset*scale_coef + ADJUST_CENTER_Y)

	# print(markerID)
	print(markerID, x_result, y_result)
	# print(y_result)
	...
	return markerID, x_result, y_result
