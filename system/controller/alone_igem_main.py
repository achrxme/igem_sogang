...
def main():
    ...
    off_data = {'there':[0, 0, 0], 'incub_plate_1':[131.1, -95.9, 13.1], 'incub_plate_2':[125, -79, -85.9], 'incub_plate_3':[89.6, -129.1, -86.2], 'incub_closed_handle':[290.5, -57.6, 42.5],
    ...
    def grip(mode, pos_align, offset_name, approach, return_or_not, test_or_not) :
        ...
        while(1) :
            ...
            if approach == '+x' :
                rb.move(pos_precise.offset(dx = off_data[offset_name][0]-200))
                rb.line(pos_precise)
            elif approach == '-x' :
                rb.move(pos_precise.offset(dx = off_data[offset_name][0]+200))
                rb.line(pos_precise)
            elif approach == '+y' :
                rb.move(pos_precise.offset(dy = off_data[offset_name][1]-200))
                rb.line(pos_precise)
            elif approach == '-y' :
                rb.move(pos_precise.offset(dy = off_data[offset_name][1]+200))
                rb.line(pos_precise)
                ...
                print('approach incub 1')
                rb.move(pos_precise.offset(dx = off_data[offset_name][0]-220, dy = off_data[offset_name][1]+200))
                print('approach incub 2')
                rb.line(pos_precise)
            else :
                rb.move(pos_precise)
        ...
        if return_or_not == 1:
            if approach == '+x' :
                rb.line(pos_precise.offset(dx = off_data[offset_name][0]-200))
            elif approach == '-x' :
                rb.line(pos_precise.offset(dx = off_data[offset_name][0]+200))
            elif approach == '+y' :
                rb.line(pos_precise.offset(dy = off_data[offset_name][1]-200))
            elif approach == '-y' :
                rb.line(pos_precise.offset(dy = off_data[offset_name][1]+200))
            elif approach == '-z':
                rb.line(pos_precise.offset(dz = off_data[offset_name][2]+20))
            elif approach == 'incub':
                rb.line(pos_precise.offset(dx = off_data[offset_name][0]-220, dy = off_data[offset_name][1]+200))
            rb.move(pos_align)
    ...
    def incubator_motion(mode, radii) :
        ...
        incub_handle_posture = posture_change(pos_incub_base, 'front_top', 0)
        ...
        if mode == 'open':
            ...
            rb.move(get_world_test_pos)
            rb.move(pos_closed_handle.offset(dz = 100))
            rb.move(pos_closed_handle.offset(dy = -55 ,dz = 100))
            rb.move(pos_closed_handle.offset(dz = 100))

            rb.move(pos_closed_handle.offset(dy = 160, dz = 150))
    ...
    def plate_open(mode, offset_name) :
        ...
        cap_release_position = target_positon.offset(dy=-150, dz = -115)
        ...
        if mode == 'open' :
            ...
            grip('grip', cap_release_position, 'there', '-z', 0, 0)

            rb.line(cap_release_position.offset(dz=100))

            grip('release',plate_open_posture, offset_name, '-z', 0, 0)
            rb.line(target_positon.offset(dz = 50))
        else :
            print('ERROR !')
        
        rb.move(plate_open_posture)
    
    def cap_open_2(mode, solution_name) :
        rb.home()
        ...
        j6_cw = MotionParam(ik_solver_option = 0x11222222)
        j6_ccw = MotionParam(ik_solver_option = 0x11333333)

        if mode == 'open':
            rb.move(pos_front_state_cap)

            for i in range(repeat_time):
                rb.motionparam(j6_ccw)
                rb.move(pos_front_state_cap)
                dout(16, grip_code['basic_grip'])
                rb.sleep(1)
                rb.motionparam(j6_cw)
            ...
            rb.move(pos_back_state_cap.offset(dz=100))
    ...
    def pipette_motion(pos_fluid, off_fluid, pos_goal, off_goal, pipet_type, init_or_not, mid_or_not, fin_or_not):
        ...
        if mid_or_not == 1:
            ...
            if init_or_not == 1:
                ...
                rb.move(near_above_fluid)
                above_fluid = offset_position(near_above_fluid, off_fluid, 0)
                adjusted_above_fluid = above_fluid.offset(dx = 15, dy = -10)
                rb.move(adjusted_above_fluid)
                rb.line(adjusted_above_fluid.offset(dz = -100))
    ...
    def mix(mix_base, repeat, amplitude):
        ...
        m_mix = MotionParam(jnt_speed = 35)
