...
def main():
    ...
    m_basic = MotionParam( lin_speed = 25, jnt_speed = 35)
    ...
    off_data = {'there':[0, 0, 0], 'incub_plate_1':[131.1, -95.9, 13.1], 'incub_plate_2':[131.5, -74.3, -85.9], 'incub_plate_3':[89.6, -129.1, -86.2], 'incub_closed_handle':[290.5, -57.6, 42.5],
                'pipet_thick_handle':[102, 142, -70.2], 'pipet_thick_tip':[39.9, 334, 37.2], 'pipet_thin_handle':[-109.5, 59, -75.7], 'pipet_thin_tip':[-37.8, -221, -32],
                'PBS_cap':[103.2, 203.4, 55.5], 'medium_cap':[9.1, 203.4, 55.5], 'conical_cap':[-96.5, 203.4, 55.5], 'trypsin_cap':[102.2, 104.4, 54.6],
                'above_PBS':[78.8, 59.2, 20], 'above_medium':[-28, 70, 20], 'above_conical':[-121.2, 59.2, 20], 'above_trypsin':[77.5, -39.7, -36],
    ...
    pos_tip_remove_marker = Position(pos_x[4], pos_y[4], test_height, posture['front_top'][0], posture['front_top'][1], posture['front_top'][2])
    pos_pipet_thin_marker = Position(pos_x[5], pos_y[5], test_height, posture['front_top'][0], posture['front_top'][1], posture['front_top'][2])
    pos_centri_marker = Position(pos_x[6], pos_y[6], test_height, posture['front_top'][0], posture['front_top'][1], posture['front_top'][2])
    ...
    def cap_open(mode, solution_name) :
        ...
        if mode == 'open':
            ...
            for i in range(repeat_time):
                dout(16, grip_code['basic_grip'])
                rb.sleep(1)
                rb.move(joint_solution_cap.offset(dj6=0))
                rb.move(joint_solution_cap.offset(dj6=175))
                dout(16, grip_code['basic_release'])
                rb.sleep(1)
                rb.move(joint_solution_cap.offset(dj6=-170))
                dout(16, grip_code['basic_grip'])
                rb.sleep(1)
                rb.move(joint_solution_cap.offset(dj6=0))
                dout(16, grip_code['basic_release'])
            ...
            rb.move(pos_solution_cap.offset(dz=30))
            grip('release', pos_cap_release, 'there', 0, 0, 0)
            rb.line(pos_cap_release.offset(dz=20))
    ...
    def pipette_motion(pos_fluid, off_fluid, pos_goal, off_goal, pipet_type, init_or_not, mid_or_not, fin_or_not):
        ...
        if fin_or_not == 1 :
            ...
            four_posture = posture_change(pos_tip_remove_base, 'tip_remove_ver', 0)
    ...
    pos_tip_remove_base =pos_tip_remove_marker.offset(dz = -200)
    pos_pipet_thin_base = pos_pipet_thin_marker.offset(dx = 100, dz = -200)
    pos_centri_base = pos_centri_marker.offset(dz = -200)
